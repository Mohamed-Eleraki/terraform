...
resource "google_compute_instance_template" "eraki_uscen1a_inst_temp_test_1001" {
  ...
  name_prefix  = "${var.company_name}-${var.region_short}-inst-temp-${var.solution_name}-"
  ...
  metadata = {
    startup-script = <<EOF
            ...
            sudo echo "Hello from Hostname: $(hostname)" > /var/www/html/index.html && echo "IP Address: $(hostname -I | awk '{print $1}')" >> /var/www/html/index.html
  ...
  tags = ["http-server", "load-balanced"] # firewall tag definition
...
resource "google_compute_health_check" "eraki_uscen1_health_check_test_1001" {
  ...
  name                = "${var.company_name}-${var.region_short}-health-check-${var.solution_name}"
  ...
  http_health_check {
    ...
    request_path = "/"
    port         = "80" # specify the port freely
  }
...
resource "google_compute_region_instance_group_manager" "eraki_uscen1_managed_inst_group_test_1001" {
  ...
  name               = "${var.company_name}-${var.region_short}-managed-inst-group-${var.solution_name}"
  region             = var.region
...
resource "google_compute_region_autoscaler" "eraki_uscen1_auto_scaler_test_1001" {
  ...
  name   = "${var.company_name}-${var.region_short}-auto-scaler-${var.solution_name}"
  region = var.region
